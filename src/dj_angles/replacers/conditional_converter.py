"""Clean implementation of dj-if/dj-else processing based on first principles.

Core insight: If-else matching is fundamentally about DOM sibling relationships.
An `else` belongs to an `if` if and only if they share the same parent.
...
def convert_conditionals(html: str, prefix: str | None = None) -> str:
    ...
    _link_chains(elements, html, prefix)
...
def _link_chains(elements: list[ConditionalElement], html: str, prefix: str) -> None:
    ...
    for i, elem in enumerate(elements):
        if elem.type == "if":
            ...
            match = _find_preceding_sibling(elem, elements[:i], html)

            if match:
                elem.chain_id = match.chain_id
                match.next_in_chain = elem
            else:
                # Tests expect AssertionError for orphaned elif/else
                # Match exact error message from existing tests
                attr_name = elem.attr_match.group(1)
                raise AssertionError(f"Invalid use of {attr_name} attribute")


def _find_preceding_sibling(
    elem: ConditionalElement, candidates: list[ConditionalElement], html: str
...
def _apply_atomic_edits(elements: list[ConditionalElement], html: str) -> str:
    ...
    for elem in elements:
        ...
        if is_closing_tag and elem.type in ("endif", "fi"):
            ...
            start_tag = "{% else %}"
        elif elem.type == "endif" or elem.type == "fi":
            start_tag = "{% endif %}"
        else:
            continue
