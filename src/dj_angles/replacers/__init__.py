...
from dj_angles.replacers.attributes import replace_conditionals, replace_loops, replace_values
...
def convert_template(html: str, *, origin=None) -> str:
    ...
    # 2. Replace for-loops, e.g. `<li dj-for="i in items">`
    html = replace_loops(html)

    # 3. Replace variables, e.g. `{{ foo or bar }}`
    html = replace_variables(html)

    # 4. Replace value attributes, e.g. `<div dj-value="request.user">`
    html = replace_values(html)

    # 5. Replace tags, e.g. `<dj-include />`
